...
public class WheelView extends View {
     ...
     * Items text color
     */
    private static final int ITEMS_TEXT_COLOR = 0xFFB8B8B8;
    ...
    private StaticLayout headLabelLayout;
    ...
    public void setLabel(int id) {
        setLabel(getContext().getString(id));
    }

    public void setheadLabel(String newHeadLabel) {
        if (headLabel == null || !headLabel.equals(newHeadLabel)) {
            headLabel = newHeadLabel;
            headLabelLayout = null;
            invalidate();
        }
    }

    public void setheadLabel(int id) {
        setheadLabel(getContext().getString(id));
    }
    ...
    private void initResourcesIfNecessary() {
        ...
        if (valuePaint == null) {
            ...
            valuePaint.setShadowLayer(0.1f, 0, 0.1f, 0xFF00FF0);
        }
...
*/
    ...
    private int calculateLayoutWidth(int widthSize, int mode) {
        ...
        if (headLabel != null && headLabel.length() > 0) {
            headLabelWidth = (int) Math.ceil(Layout.getDesiredWidth(headLabel,
                    valuePaint));
        }
        ...
        if (mode == MeasureSpec.EXACTLY) {
            ...
            width = itemsWidth + labelWidth + 2 * PADDING + headLabelWidth;
            if (labelWidth > 0) {
                width += LABEL_OFFSET;
            }
            if (headLabelWidth > 0) {
        ...
        if (recalculate) {
            ...
            if (pureWidth <= 0) {
                itemsWidth = labelWidth = headLabelWidth = 0;
            }
            if (labelWidth > 0) {
...
//			Log.e("TAG" , "---------------重新计算  itemsWidth="+itemsWidth+"   labelWidth="+labelWidth+"   headLabelWidth=" +headLabelWidth);
        }

        if (itemsWidth > 0) {
            createLayouts(itemsWidth, labelWidth, headLabelWidth);
        }
     ...
     */
    private void createLayouts(int widthItems, int widthLabel, int widthHeadLabel) {
        ...
        if (widthHeadLabel > 0) {
            if (headLabelLayout == null || headLabelLayout.getWidth() > widthHeadLabel) {
                headLabelLayout = new StaticLayout(headLabel, valuePaint, widthHeadLabel,
                        Layout.Alignment.ALIGN_NORMAL, 1,
                        ADDITIONAL_ITEM_HEIGHT, false);
            } else {
                headLabelLayout.increaseWidthTo(widthHeadLabel);
            }
        }
    }
    ...
    private void drawValue(Canvas canvas) {
        ...
        //draw headLabel
        if (headLabelLayout != null) {
            canvas.save();
            canvas.translate(itemsWidth - headLabelWidth - itemHeight / 2 - 2 * LABEL_OFFSET, bounds.top);
            headLabelLayout.draw(canvas);
            canvas.restore();
        }
        // draw current value
        if (valueLayout != null) {
    ...
    private void drawCenterRect(Canvas canvas) {
        ...
        p.setStrokeWidth(2);
        canvas.drawLine(30, center - offset, getWidth() - 30, center - offset, p);
        canvas.drawLine(30, center + offset, getWidth() - 30, center + offset, p);
    }
